param(
    ...
    [string]$NotebookSlug = "dermascanai-phase-4-mobilenet-hda"
)
...
$sourceFiles = @(
    ...
    "convnext_baseline.py", "skinnet_hda.py", "mobilenet_hda.py",
    "train_mobilenet_hda.py", "evaluate_mobilenet_hda.py",
    "verify_mobilenet_hda.py", "measure_mobilenet_efficiency.py",
    "requirements.txt", "requirements_phase3.txt"
)
$encoded = [ordered]@{}
...
if (-not (Test-Path -LiteralPath $splitPath -PathType Leaf)) {
    throw "Frozen Phase 1 split is missing: $splitPath"
}
...
assert split['lesion_id'].nunique() == 7470
...
print('Clean-room Phase 4 code written to:', WORK_DIR)
print('Frozen Phase 1 split restored:', split['split'].value_counts().to_dict())
...
subprocess.run([
    sys.executable, 'verify_mobilenet_hda.py', '--dataset-root', dataset_root,
...
dataset_root = open('dataset_root.txt', encoding='utf-8').read().strip()
subprocess.run([
    sys.executable, 'train_mobilenet_hda.py', '--dataset-root', dataset_root,
    '--artifacts-dir', 'artifacts', '--output-dir', 'outputs/mobilenet_hda',
...
dataset_root = open('dataset_root.txt', encoding='utf-8').read().strip()
subprocess.run([
    sys.executable, 'evaluate_mobilenet_hda.py', '--dataset-root', dataset_root,
    '--artifacts-dir', 'artifacts',
    '--checkpoint', 'outputs/mobilenet_hda/best_mobilenet_hda.pt',
    '--output-dir', 'outputs/mobilenet_hda/test', '--batch-size', '32',
    '--num-workers', '2', '--seed', '42', '--device', 'cuda',
], check=True)
subprocess.run([
    sys.executable, 'measure_mobilenet_efficiency.py',
    '--checkpoint', 'outputs/mobilenet_hda/best_mobilenet_hda.pt',
    '--output', 'outputs/mobilenet_hda/efficiency.json', '--device', 'cuda',
    '--warmup-runs', '30', '--timed-runs', '100',
], check=True)

from pathlib import Path
print('Final Phase 4 output files:')
for path in sorted(Path('outputs/mobilenet_hda').rglob('*')):
...
$request = @{
    ...
    newTitle = "DermaScanAI Phase 4 MobileNet-HDA"
