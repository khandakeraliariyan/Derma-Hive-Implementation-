"""Forward/backward and tensor-shape verification for Phase 3."""
...
from convnext_baseline import balanced_class_weights, set_seed
from dataset_preparation import prepare_ham10000
from skinnet_hda import FocalLoss, SkinNetHDA
from train_skinnet_hda import choose_device, make_loader


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify Phase 3 forward/backward path")
...
def verify_tensors(model, images, metadata, labels, criterion) -> None:
    ...
    expected = {
        "image": (expected_batch, 3, 224, 224),
        "convnext": (expected_batch, 768, 7, 7),
        "se": (expected_batch, 768, 7, 7),
        "spatial_attention": (expected_batch, 768, 7, 7),
        "tokens": (expected_batch, 49, 768),
        "transformer": (expected_batch, 49, 768),
        "pooled_image_feature": (expected_batch, 768),
        "metadata": (expected_batch, metadata.shape[1]),
        "fused_feature": (expected_batch, 768 + metadata.shape[1]),
        "logits": (expected_batch, 7),
    }
...
def main() -> None:
    ...
    model = SkinNetHDA(len(prepared.metadata_feature_names), pretrained=True).to(device)
