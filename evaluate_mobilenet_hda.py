"""Evaluate our separate Phase 4 MobileNetV3-Large HDA model."""
...
from train_mobilenet_hda import choose_device, collect_predictions, make_loader


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Phase 4 MobileNet-HDA")
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=Path("outputs/mobilenet_hda/test"))
...
def main() -> None:
    ...
    if checkpoint.get("class_to_index") != CLASS_TO_INDEX:
        raise ValueError("Checkpoint class mapping does not match the pipeline")
    if checkpoint["metadata_feature_names"] != prepared.metadata_feature_names:
        raise ValueError("Checkpoint metadata features do not match preprocessing")
    ...
    loader = make_loader(prepared.test_dataset, args.batch_size, False,
                         args.num_workers, args.seed + 2, device.type == "cuda")
